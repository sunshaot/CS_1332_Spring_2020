...
public class LinkedQueue<T> {
    ...
    public void enqueue(T data) {
        if (data == null) {
            throw new IllegalArgumentException("Error, cannot add null to queue!");
        } else {
            if (size == 0) {
                head = new LinkedNode(data);
                tail = head;
            } else {
                LinkedNode newTail = new LinkedNode(data);
    ...
    public T dequeue() {
        if (size == 0) {
            throw new NoSuchElementException("Error, cannot remove element from empty queue!");
        } else {
            T data = head.getData();
            head = head.getNext();
            size--;
    ...
    public T peek() {
        if (size == 0) {
            throw new NoSuchElementException("Error, cannot get peek from empty queue!");
        } else {
            return head.getData();
        }
    }
